...
//11.binarySearch
int binarySearch(std::vector<int>& data,int targetVal){
	int left = 0;
	int right = data.size();
...
std::vector<int> createData_int(size_t dataSize){
	...
	for(size_t i = 0;i < dataSize;i++){
		random_data.push_back(rand()%10000 + 1);
	}
...
std::vector<double> createData_double(size_t dataSize){
	...
	for(size_t i = 0;i < dataSize;i++){
		random_data.push_back(1.0+(double)rand()/RAND_MAX*9999.0);
	}
	std::cout << "\nCreated double Data\n";
	return random_data;
}
...
int main(){
	...
	int target_value;
	std::cout << "Enter integer to be target value for search : ";
...
//10
	...
	if(search_indexOfValue1 == -1) std::cout << "not found\n";
	else{
		std::cout << "index found : " << search_indexOfValue1 << "\n";
	}
	std::cout << "Time taken for linearSearch : " << duration1.count() << " microseconds\n";
//11
//	std::vector<int> testData_int11 = createData_int(input_number);
	auto start2 = std::chrono::steady_clock::now();
	int search_indexOfValue2 = linearSearch(testData_int10,target_value);
	auto end2 = std::chrono::steady_clock::now();
	auto duration2 = std::chrono::duration_cast<std::chrono::microseconds>(end2-start2);
	if(search_indexOfValue2 == -1) std::cout << "not found\n";
	else{
		std::cout << "index found : " << search_indexOfValue2 << "\n";
	}
	std::cout << "Time taken for binarySearch : " << duration2.count() << " microseconds\n";
	return 0;
}
